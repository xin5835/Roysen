...
test1 %>% 
  ...
  geom_jitter(aes(color = I("red")))+
  geom_boxplot(aes(group = month, alpha = 0.5))

test1 %>% 
  ggplot(aes(wday, distance))+
  geom_jitter(aes(color = I("red")))+
  geom_boxplot(aes(group = wday, alpha = 0.5))

test1 %>% 
  ggplot(aes(hour, distance))+
  geom_jitter(aes(color = I("red")))+
  geom_boxplot(aes(group = hour, alpha = 0.5))
```
不论月、周、日，纽约出租车用户的行驶距离差异不大，组间四分位数几乎没有大的变化。
